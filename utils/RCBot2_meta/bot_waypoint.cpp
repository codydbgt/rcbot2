...
{
	const int iWpt = CWaypointLocations::GetCoverWaypoint(m_pBot->getOrigin(), vCoverOrigin,NULL);
									  ...
									  int iConditions, int iDangerId )
{
	if ( bRestart )
	{
		...
		*bFail = false;
		...
		CWaypoint* pGoalWaypoint = CWaypoints::getWaypoint(m_iGoalWaypoint);
...
{
	const int iwpt = CWaypointLocations::NearestWaypoint(vOrigin,100,-1,true,false,true,NULL,false,m_pBot->getTeam());
...
void CWaypoint :: draw ( edict_t *pEdict, bool bDrawPaths, unsigned short int iDrawType )
{
	...
	case DRAWTYPE_BELIEF:
		{
			if ( CClients::clientsDebugging(BOT_DEBUG_NAV) )
			{
				...
				if ( pClient )
				{
					...
					if ( pBot )
					{
						const char belief = (char)(int)pBot->getNavigator()->getBelief(CWaypoints::getWaypointIndex(this));
	...
	case DRAWTYPE_DEBUGENGINE:
		...
		// draw radius
		if ( m_fRadius && distanceFrom(CBotGlobals::entityOrigin(pEdict)) < 400.0f )
		{
...
{
	const int iWpt = CWaypointLocations::GetCoverWaypoint(vPlayerOrigin,vPinchOrigin,NULL,&vPinchOrigin);
...
CWaypoint *CWaypoints :: nearestPipeWaypoint ( Vector vTarget, Vector vOrigin, int *iAiming )
{
	...
	const short int iTarget = (short int)CWaypointLocations::NearestWaypoint(vTarget,BLAST_RADIUS,-1,true,true);
	...
	for (short int i = 0; i < numwaypoints; i ++ )
	{
		...
		if ( pTable->GetVisibilityFromTo(iTarget,i) )
		{
			...
			{				
				...
				CWaypoint* pTempj = CWaypoints::getWaypoint(j);
